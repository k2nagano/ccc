...
class SonarWidget(QWidget):
    ...
    def update_image(self):
        ...
        self.center = QPointF(w / 2, h)
        self.radius = h * 0.9

        self.clip_path = QPainterPath()
        rect = QRectF(self.center.x() - self.radius, self.center.y() - self.radius, self.radius*2, self.radius*2)
        self.clip_path.moveTo(self.center)
        self.clip_path.arcTo(rect, 90 - self.swath/2, self.swath)
        self.clip_path.closeSubpath()
        ...
        valid = (r >= 0) & (r <= self.radius) & (theta >= -self.swath/2) & (theta <= self.swath/2)

        r_idx = (r / self.radius * (n_range - 1)).astype(np.int32)
        theta_idx = ((theta + self.swath/2) / self.swath * (n_beam - 1)).astype(np.int32)

        r_idx = np.clip(r_idx, 0, n_range-1)
        theta_idx = np.clip(theta_idx, 0, n_beam-1)

        intensity = np.zeros((h, w), dtype=np.uint8)
        intensity[valid] = self.frame[theta_idx[valid], r_idx[valid]]
        ...
        color_array = min_rgb + (max_rgb - min_rgb) * intensity[:, :, None] / 255.0
        color_array = np.clip(color_array, 0, 255).astype(np.uint8)

        ptr = self.image.bits()
        arr = np.frombuffer(ptr, dtype=np.uint8).reshape((h, w, 4))
        arr[:, :, 0:3] = color_array[:, :, ::-1]
        arr[:, :, 3] = np.where(valid, 255, 0)
